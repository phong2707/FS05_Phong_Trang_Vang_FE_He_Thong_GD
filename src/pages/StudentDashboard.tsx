...
const deadlines = [
  ...
  {
    task: "Data Structure Assignment",
    date: "Due in 5 days",
    bg: "bg-teal-50",
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-teal-100 p-6">
                ...
                <div className="space-y-3">
                  {courses.map((course, i) => (
                    <div
                      key={i}
                      className="p-4 border border-gray-100 rounded-xl hover:border-teal-200 hover:bg-teal-50/30 transition-all duration-200 group"
                    >
                      <div className="flex items-center justify-between mb-2.5">
                        <div>
                          <p className="font-semibold text-gray-900 text-sm group-hover:text-teal-800 transition-colors">
                            {course.name}
                          </p>
                        ...
                        <span
                          className={`text-sm font-black ${getProgressTextColor(course.progress)}`}
                        >
                          {course.progress}%
                        </span>
                      </div>
                      <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                        <div
                          ...
                          style={{
                            ...
                            background: getProgressGradient(course.progress),
                          }}
                        />
                      </div>
                    </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-teal-100 p-6">
                ...
                <div className="space-y-3 mb-5">
                  {deadlines.map((item, i) => (
                    <div
                      key={i}
                      className={`p-3 ${item.bg} border ${item.border} rounded-xl flex items-start gap-3`}
                    >
                      <span
                        className={`w-2 h-2 rounded-full flex-shrink-0 mt-1.5 ${item.dot}`}
                      />
                      <div>
                        <p
                        ...
                        <p className={`text-xs ${item.dateColor} mt-0.5`}>
                          {item.date}
                        </p>
                      </div>
                    </div>
                ...
                {/* Achievement */}
                <div
                  className="p-4 rounded-xl border border-emerald-200 flex items-start gap-3"
                  style={{
