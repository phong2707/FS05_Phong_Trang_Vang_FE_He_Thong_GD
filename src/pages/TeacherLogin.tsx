      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-24 bg-stone-50">
        <div className="w-full max-w-md space-y-8 bg-white p-10 rounded-3xl shadow-lg border" style={{ borderColor: '#e2e8f0' }}>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6">
              {/* 🆕 Sử dụng GoogleLogin component chính thức từ @react-oauth/google */}
              ...
              {/* Nếu bạn muốn sử dụng button tùy chỉnh thay vì component Google, dùng cái này: */}
              {/* 
              <button
                type="button"
                disabled={googleLoading}
                onClick={() => {
                  // Trigger Google sign-in dialog nếu bạn muốn custom button
                }}
                className="w-full flex justify-center items-center py-3 px-4 border rounded-xl shadow-sm text-sm font-semibold bg-white hover:bg-gray-50 transition-colors duration-200 disabled:opacity-70 disabled:cursor-not-allowed"
                style={{ borderColor: '#e2e8f0', color: '#0f172a' }}
              >
                ...
                {googleLoading ? 'Đang xử lý...' : 'Đăng nhập bằng Gmail'}
              </button>
              */}
            </div>
          </div>
